...
On startup, the `DataInitializer` seeds **5 products, 3 warehouses, and inventory** so you can test APIs immediately — no manual setup needed.

### Seeded Data at a Glance

| Products (IDs 1–5) | Warehouses (IDs 1–3) |
|---|---|
| MacBook Pro (ELECTRONICS, $2499.99) | Bangalore Hub — all 5 products |
| iPhone 15 (ELECTRONICS, $999.99) | Mumbai Hub — laptops, phones, headphones |
| Polo Shirt (CLOTHING, $49.99) | Delhi Hub — shirts, shoes |
...
4. [DataInitializer — Seed Data on Startup](#4-datainitializer--seed-data-on-startup)
5. [API Reference with curl Examples](#5-api-reference-with-curl-examples)
6. [End-to-End Flow Walkthrough](#6-end-to-end-flow-walkthrough)
7. [Design Patterns Used & Why](#7-design-patterns-used--why)
8. [Key Concepts to Explain in Interview](#8-key-concepts-to-explain-in-interview)
9. [Thread Safety — How & Why](#9-thread-safety--how--why)
10. [Edge Cases Handled](#10-edge-cases-handled)
11. [Order State Machine](#11-order-state-machine)
12. [Stock Lifecycle](#12-stock-lifecycle)
13. [How the Layers Connect — Request-to-Response Trace](#13-how-the-layers-connect--request-to-response-trace)
14. [Common Follow-Up Questions & Answers](#14-common-follow-up-questions--answers)
15. [What to Say in Last 5 Minutes](#15-what-to-say-in-last-5-minutes)
16. [Checklist Before Interview](#16-checklist-before-interview)
...
├── InventoryManagementSystem.java      ← Standalone demo (compile & run without Spring)
│
├── config/                             ← Configuration & startup
│   └── DataInitializer.java            CommandLineRunner — seeds products, warehouses, inventory
│
...
## 4. DataInitializer — Seed Data on Startup

The `DataInitializer` class implements `CommandLineRunner`, which Spring Boot executes automatically after the application context loads. It uses the existing service layer to create sample data.
...
Products:
  ...
  2. iPhone 15        — ELECTRONICS  $999.99
  3. Polo Shirt       — CLOTHING     $49.99
  4. Sony WH-1000XM5  — ELECTRONICS  $349.99
  5. Running Shoes    — SPORTS       $129.99

Warehouses:
  1. Bangalore Hub  — Whitefield, Bangalore
  2. Mumbai Hub     — Andheri, Mumbai
  3. Delhi Hub      — Gurugram, Delhi NCR

Inventory (warehouseId → productId → quantity):
  Bangalore Hub:  MacBook(50), iPhone(100), Polo(200), Sony(75), Shoes(60)
  Mumbai Hub:     MacBook(30), iPhone(80), Sony(50)
  Delhi Hub:      Polo(150), Shoes(90)
```
...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        // Uses existing service methods — same path as API calls
...
**Key point**: The initializer goes through the *same service layer* as API calls. This means all validations, ID generation, and business logic apply — it's not a hack, it's a proper data setup.

### Why This Pattern Matters for Interviews

- **Instant demo**: Run `./gradlew bootRun` → data is ready → fire curl commands immediately
- **No external dependencies**: No database, no SQL scripts, no manual Postman setup
- **Proves your code works**: If DataInitializer runs without errors, your entire service layer is validated

---

## 5. API Reference with curl Examples
...
Data is **already seeded by DataInitializer**. Just run `./gradlew bootRun` and fire these:

```bash
# Step 1: Verify seeded data
curl -s localhost:8080/api/products | python3 -m json.tool
# → 5 products (MacBook, iPhone, Polo Shirt, Sony WH-1000XM5, Running Shoes)

curl -s localhost:8080/api/warehouses | python3 -m json.tool
# → 3 warehouses (Bangalore, Mumbai, Delhi)

# Step 2: Check inventory in Bangalore Hub
curl -s localhost:8080/api/inventory/warehouse/1 | python3 -m json.tool
# → MacBook(50), iPhone(100), Polo(200), Sony(75), Shoes(60)

# Step 3: Check MacBook availability across all warehouses
curl -s localhost:8080/api/inventory/product/1 | python3 -m json.tool
# → Bangalore: 50, Mumbai: 30

# Step 4: Check if we can order 10 MacBooks
curl -s "localhost:8080/api/inventory/check?productId=1&quantity=10"
# → available: true
...
curl -s -X POST localhost:8080/api/orders -H "Content-Type: application/json" \
  -d '{"customerName":"Puja","items":[{"productId":1,"quantity":3},{"productId":2,"quantity":5}]}'
# → status: PENDING, totalAmount: $12,499.92

# Step 6: Check inventory — reserved shows up
curl -s localhost:8080/api/inventory/warehouse/1 | python3 -m json.tool
# → MacBook: total=50, reserved=3, available=47
# → iPhone: total=100, reserved=5, available=95
...
curl -s localhost:8080/api/inventory/warehouse/1 | python3 -m json.tool
# → MacBook: total=47, reserved=0, available=47

# Step 9: Ship → Deliver (full lifecycle)
...
curl -s -X POST localhost:8080/api/orders -H "Content-Type: application/json" \
  -d '{"customerName":"Test","items":[{"productId":1,"quantity":99999}]}'
# → 400: "Insufficient stock for product: MacBook Pro (id=1)"
...
# --- Order cancellation (stock released back) ---
curl -s -X POST localhost:8080/api/orders -H "Content-Type: application/json" \
  -d '{"customerName":"Cancel Test","items":[{"productId":3,"quantity":10}]}'
curl -s -X PUT localhost:8080/api/orders/2/cancel
# → status: CANCELLED, Polo Shirt stock restored to original

# --- Validation error ---
curl -s -X POST localhost:8080/api/products -H "Content-Type: application/json" \
  -d '{"name":"","category":"ELECTRONICS","price":-5}'
# → 400: "name: must not be blank, price: must be greater than 0"

# --- Create new product + add to inventory ---
curl -s -X POST localhost:8080/api/products -H "Content-Type: application/json" \
  -d '{"name":"iPad Air","category":"ELECTRONICS","price":599.99,"description":"M2 chip"}'
curl -s -X POST localhost:8080/api/inventory/add-stock -H "Content-Type: application/json" \
  -d '{"warehouseId":1,"productId":6,"quantity":40}'
```

---

## 7. Design Patterns Used & Why

### 7a. Builder Pattern (Lombok @Builder)
...
### 7b. Saga Pattern (Compensating Transactions)
...
### 7c. State Machine (Order Lifecycle)
...
### 7d. Repository Pattern (In-Memory)
...
### 7f. Global Exception Handling (@RestControllerAdvice)
...
## 8. Key Concepts to Explain in Interview

### 8a. Reserve → Confirm → Release Pattern
...
### 8c. Why ConcurrentHashMap, Not HashMap
...
### 8d. Why AtomicLong for ID Generation
...
## 13. How the Layers Connect — Request-to-Response Trace

Understanding how a single API call flows through the layers is critical for interviews. Here's what happens when you place an order:

### `POST /api/orders` — Full Trace

```
1. HTTP Request hits OrderController.placeOrder()
   ↓ @Valid validates PlaceOrderRequest (customerName not blank, items not empty)
   ↓ @RequestBody deserializes JSON → PlaceOrderRequest record

2. Controller delegates to OrderService.placeOrder(req)
   ↓ (controller has ZERO logic — it just delegates and wraps the response)

3. OrderService iterates over each OrderItemRequest:
   ↓ For each item:
   │  a. productService.getById(productId)
   │     → ConcurrentHashMap lookup → returns Product or throws ResourceNotFoundException
   │  b. inventoryService.reserveStock(productId, quantity)
   │     → Scans all Inventory entries for this product
   │     → Finds first warehouse with enough available stock
   │     → Calls inventory.reserve(qty) — SYNCHRONIZED method
   │     → Returns warehouseId, or null if insufficient stock
   │  c. If null → throws BadRequestException → triggers SAGA ROLLBACK
   │     → All previously reserved items get inventoryService.releaseStock()
   │  d. Builds OrderItem with price snapshot (unitPrice = product.getPrice())

4. If all items reserved successfully:
   ↓ Builds Order with AtomicLong.getAndIncrement() for unique ID
   ↓ Sets status = PENDING
   ↓ Stores in ConcurrentHashMap

5. Control returns to OrderController:
   ↓ Wraps Order in ApiResponse.success(order, "Order placed")
   ↓ Returns ResponseEntity with HttpStatus.CREATED (201)

6. If any exception was thrown:
   ↓ GlobalExceptionHandler catches it
   ↓ Maps to appropriate HTTP status (400, 404, 500)
   ↓ Returns consistent ApiResponse.error(message)
```

### Class Dependency Graph

```
Controller → Service → Service (cross-service calls)
                    → Model (data objects)
                    → ConcurrentHashMap (storage)

OrderController
  └── OrderService
        ├── ProductService    → Map<Long, Product>
        └── InventoryService  → Map<String, Inventory>
              ├── ProductService    (to look up product names)
              └── WarehouseService  → Map<Long, Warehouse>
```

### Spring DI Wiring

All services are `@Service` beans. Controllers use `@RequiredArgsConstructor` (Lombok) for constructor injection — no `@Autowired` annotations needed.
...
## 14. Common Follow-Up Questions & Answers
...
## 15. What to Say in Last 5 Minutes
